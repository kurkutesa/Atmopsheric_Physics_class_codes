...
diagnostic = 2 # 1 for height tendency equation, 2 for traditional omega, 3 for Sutcliffe-Trenberth
levels_lowerupper = [70000,30000] # Set the level(s) you want to plot vorticity at
...
levelindex_lower = np.ravel(levs==levels_lowerupper[0])
levelindex_upper = np.ravel(levs==levels_lowerupper[1])
...
if diagnostic == 1:
   temp_lower = f.variables['TMP_P0_L100_GLL0'][levelindex_lower,::-1,:].squeeze()
   temp_upper = f.variables['TMP_P0_L100_GLL0'][levelindex_upper,::-1,:].squeeze()   
elif diagnostic == 2:
...
if diagnostic == 1: # height tendency
   # Vertical  vorticity
   var_mid = wm.vertical_vorticity_latlon(uin_mid, vin_mid, lats, lons, 1)   
   
   var_lower = temp_lower
   var_upper = temp_upper
   
   # for differential thermal advection
   ...
   #  vorticity advection level middle level
   var_adv_mid = wm.hadvection_latlon(uin_mid, vin_mid, var_mid, lats, lons)      
...
elif diagnostic == 3: # sutcliffe-trenberth omega
   # putstuffhere
   
   # (1) Compute the thickness using geopotential heights above and below the level you computed vorticity in step 1
   # (2) Compute the thermal wind using the function thermal_wind_sphere in weather_modules.py
   # (3) Compute advection of the thermal wind.  You will need to compute the horizontal vorticity gradient using the function gradient_sphere in weather_modules.py, then manually compute the advection.  
   #     Be sure to use the thermal wind and not the geostrophic wind in the advection calculation.
   ...
   # dummy variables
   var_lower = var_mid   
   var_adv_lower = var_mid
   var_adv_upper = var_mid

   uin_upper = thermal_wind_u
   vin_upper = thermal_wind_v
...
level_hlower = levels_lowerupper[0] / 100 # Convert level to hPa for title
level_hupper = levels_lowerupper[1] / 100
...
if diagnostic == 1:
   ...
   cbar_min_upper = base_cntr_upper-20*cint_upper
   cbar_max_upper = base_cntr_upper+20*cint_upper
   ...
   cbar_min_mid = 10*cint_mid
   cbar_max_mid = cbar_min_mid+12*cint_mid
   ...
   base_cntr_mid = 0
   base_cntr_upper = 6180
 
   
   cbar_min_mid = 8*cint_mid
   cbar_max_mid = base_cntr_mid+26*cint_mid
   cbar_max_mid = cbar_max_mid + (cint_mid/2)
  

   cbar_min_upper = base_cntr_upper-14*cint_upper
   cbar_max_upper = base_cntr_upper+14*cint_upper
   cbar_max_upper = cbar_max_upper + (cint_upper/2)   
   ...
   # Dummy contour intervals
   cbar_min_lower = cbar_min_upper
   cbar_max_lower = cbar_max_upper
   cint_lower = cint_upper
   cbar_min_diffadv = cbar_min_upper
   cbar_max_diffadv = cbar_max_upper
   cint_diffadv = cint_upper
   
   # Set titletexts here
   titletext1 = '%s:%s hPa thickness, %s hPa avort valid %s at %s UTC' % (
        	level_hlower, level_hupper, level_hmid, dt.strftime('%d %b %Y'), dt.strftime('%H00'))
   titletext2 = '%s hPa abs. geos. vort. adv. by thermal wind valid %s at %s UTC' % (
        	level_hmid, dt.strftime('%d %b %Y'), dt.strftime('%H00'))
...
base_cntr_ghgts = 5400 # a contour close to standard atmospheric value
cint_ghgts = 60 # contour interval
cbar_min_ghgts = base_cntr_ghgts-20*cint_ghgts
cbar_max_ghgts = base_cntr_ghgts+20*cint_ghgts
cflevs_ghgts = np.arange(cbar_min_ghgts, cbar_max_ghgts+1, cint_ghgts)
cflevs_ghgts_ticks = np.arange(cbar_min_ghgts,cbar_max_ghgts,4*cint_ghgts)
...
if diagnostic == 1:
   ...
   CS = m.contour(x,y,var_upper,levels=cflevs_upper, colors='k', linestyles='solid',linewidths=2.0)
   plt.clabel(CS, inline=1, fontsize=10, fmt='%i')
   #CS = m.contour(x,y,var_lower,levels=cflevs_lower, colors='k', linestyles='solid',linewidths=2.0)
   #plt.clabel(CS, inline=1, fontsize=10, fmt='%i')   
elif diagnostic == 3:
   # put stuff here
...
if plot_barbs == "true":
   ...
   #barbs = m.barbs(xx_lower,yy_lower,uproj_lower,vproj_lower,length=5,barbcolor='k',flagcolor='r',linewidth=1.0)
...
if diagnostic == 1:
   ...
   #CS1 = m.contourf(x,y,var_mid,cmap=plt.cm.Greys,levels=cflevs_mid, extend='both',zorder=1)
        
   
   CS2 = m.contour(x, y, var_mid, cflevs_mid, colors='k', linewidths=0.5)
  
   
   CS3 = m.contour(x, y, ghgt_plot_mid, cflevs_ghgts, colors='k', linewidths=2.0)   
   clabs = ['%i K' % f for f in cflevs_ghgts_ticks]      
